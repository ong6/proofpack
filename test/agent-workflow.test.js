...
test('agent evidence and proposals never become approval; attachments, exports and UI preserve boundaries', async t => {
  ...
  for (const format of ['manifest', 'handover', 'fielddeck']) { const exported = await call('pilot.export', { pilotId, format }); const text = Buffer.from(exported.artifact.base64, 'base64').toString(); assert.ok(!text.includes('PRIVATE_AGENT_PROPOSAL_SENTINEL')); assert.ok(!text.includes('fixture.txt')); }
